    ...
    try:
        ...
        if not conversation_id:
            conversation_id = db_service.create_conversation(
                user_id, title=request.query[:50]
            )
        ...
        db_service.add_message(
            ...
            metadata={
                "chunks": [c.dict() for c in response.chunks],
...
async def update_conversation_endpoint(
    conversation_id: str, payload: dict, user_id: str = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        success = db_service.update_conversation(
            conversation_id, user_id, title=title, is_pinned=is_pinned
        )
        if not success:
            raise HTTPException(
...
async def stream_chat(
    ...
    async def event_generator():
        try:
            ...
            history_for_check = db_service.get_conversation_history(
                conv_id, user_id, limit=2
            )
            ...
            if history_for_check:
                ...
                if last_msg.get("role") == "assistant" and len(history_for_check) >= 2:
                    ...
                    if (
                        ...
                        and prev_msg.get("content") == query
                    ):
                        logger.info(
                            f"[Stream] Regeneration detected. Deleting last response..."
                        )
                        db_service.delete_message(last_msg["id"])
                        should_add_user_msg = False
                # Retry Logic
                elif (
                    last_msg.get("role") == "user" and last_msg.get("content") == query
                ):
            ...
            try:
                ...
                if "foreign key" in str(e) or "23503" in str(e) or not conv_id:
                    ...
                    conv_id = db_service.create_conversation(
                        user_id, title=query[:50], id=conv_id
                    )
                    db_service.add_message(conv_id, user_id, "user", query)
                else:
                    raise e
            ...
            history = db_service.get_conversation_history(
                conv_id, user_id, limit=actual_window_size
            )
            ...
            asyncio.create_task(
                evaluator_service.evaluate_rag_response(
                    query=query,
                    retrieved_chunks=stream_chunks,
                    generated_answer=final_content,
                    session_id=conv_id,
